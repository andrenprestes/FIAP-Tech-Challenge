...
class APITestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        app.config[
            'JWT_SECRET_KEY'] = 'sua_chave_secreta_aqui'  # Certifique-se de que a chave secreta seja a mesma usada no app
    ...
    def get_jwt_token(self):
        ...
        with app.test_client() as client:
            response = client.post('/login', json={'username': 'admin', 'password': 'senha123'})
            self.assertEqual(response.status_code, 200)
            return response.json['access_token']

    def test_login(self):
        # Teste para o endpoint de login
        response = self.app.post('/login', json={'username': 'admin', 'password': 'senha123'})
        self.assertEqual(response.status_code, 200)
    ...
    def test_producao_csv_com_token(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertTrue(
            isinstance(response.json, dict) or isinstance(response.json, list))  # Verifica se o retorno é JSON
